...
docker build.sh \
    --no-cache \
    --build.sh-arg BIN_TARGET="$BIN_TARGET" \
    --build.sh-arg FEATURES="${1:-disable-tests}" \
    --tag $TON_NODE \
    ./ton-node-se
...
docker build.sh \
    --no-cache \
    --build.sh-arg TON_NODE="$TON_NODE" \
    --build.sh-arg Q_SERVER_GITHUB_REPO="$Q_SERVER_GITHUB_REPO" \
    --build.sh-arg Q_SERVER_GITHUB_REV="$Q_SERVER_GITHUB_REV" \
    --tag $TONOS_SE \
    ./docker
